...
factor_data <- read.delim("CESC_PCA_Onset-TF2SF1.txt")
...
factor_all <- factor_data_all  %>% 
  ...
  mutate(TruthValue = fct_relevel(TruthValue, "N"))

LMM <- lmer(loading ~ Familarity*TruthValue*Electrode + (1|Sub), factor_all)
summary(PCA_LMM)

LMM_Post <-emmeans(PCA_LMM, pairwise~Familarity*TruthValue|Electrode, adjust="tukey")
summary(PCA_LMM_Post)

# save output (LMM, post-hoc results) to txt. files
sink(file = "lmm_output_N400_critical.txt")
summary(LMM)
sink(file = NULL)

sink(file = "post_output_N400_critical.txt")
summary(LMM_Post)
sink(file = NULL)
